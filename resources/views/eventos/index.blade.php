...
@section('content')
        ...
        <div class="well well-sm text-right">
            <h1 class="page-header" style="float:left; margin:auto; padding:auto;">Listado de Eventos</h1>
            <a class="btn btn-primary agregar" href='{{url('/eventos/create')}}'>Crear Nuevo Evento!</a>
        </div>
        ...
        <table id="locaciones" class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$evento->titulo}}</td>
                    <td style="font-size: 14px !important">{{$evento->descripcionEvento}}</td>
                    <td>{{$evento->tipo}}</td>
                    <td>{{$evento->categoria}}</td>
                    <td>{{$evento->nombreProductor}}</td>
                    <td>{{$evento->nombreCiudad}}</td>
                    <td>{{$evento->nombreLocacion}}</td>
                    <td>{{$evento->fecha_hora}}</td>
                    <td>{{$evento->costo}}</td>

                    <td>
                        <div class="btn-group">
                            <button type="button" class="btn btn-info dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Acciones</button>

                            <div class="dropdown-menu">
                            <a class="dropdown-item" href="/eventos/{{$evento->idEvento}}/cierre/"><i class="fas fa-balance-scale-right"></i>&nbsp; Balance</a>
                            <a class="dropdown-item" href="/eventos/{{$evento->idEvento}}/edit"><i class="fas fa-pencil-alt iconos"></i>&nbsp; Editar</a>

                            <div class="dropdown-divider"></div>
                            <form action="{{route('eventos.destroy', $evento->idEvento)}}" method="post" class="btn-group">
                                @csrf
                                @method('DELETE')
